...
This leaves participants less than a month before the submission deadline of August 9th. We hope the community can come together and submit runs by: 1) Submitting standard approaches and baselines from TREC 2020, to see how these perform on the new datasets, 2) Implement your newer approaches too that you are working on now or were developed since TREC 2020, and 3) Hopefully some participants can try hybrid approaches that are enabled by the new document and passage corpus with passage-document mapping. For example, a passage task run could start with full ranking on the document corpus, then identifying candidate passages from top documents, leading in to passage reranking.
...
Although some labels from past years may be available, and mapped to the new corpus.
...
The passage corpus is in jsonl format. Each passage has:
